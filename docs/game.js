...
let peer = null;
let conn = null;           // data connection to the other player
let isHost = false;        // host runs the engine
let engine = null;         // GameEngine instance (host only)
let gameState = null;
let selectedBenchIndex = null;
let selectedBoardCell = null;
let myRoomCode = null;
let myPlayerName = null;
let myPlayerId = 'host';   // 'host' or 'guest'
...
function joinGame() {
  ...
  peer.on('open', () => {
    console.log('[Guest] Peer opened, connecting to host:', PEER_PREFIX + roomCode);
    conn = peer.connect(PEER_PREFIX + roomCode, { metadata: { name }, reliable: true });

    conn.on('open', () => {
      console.log('[Guest] Connected to host');
      updateConnectionStatus(true);
      saveSession(roomCode, name, 'guest');
    });
    ...
    conn.on('error', (err) => {
      ...
      showError('连接失败，请检查房间号');
      btn.textContent = '🤝 加入房间'; btn.disabled = false;
    });
...
(function(){
  ...
  if(room&&room.length===4){ const inp=document.getElementById('roomCodeInput'); if(inp){inp.value=room; setTimeout(()=>inp.focus(),500);} }
})();
